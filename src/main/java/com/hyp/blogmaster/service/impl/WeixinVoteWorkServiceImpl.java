...
import com.hyp.blogmaster.pojo.vo.page.weixin.VoteDetailSimpleVO;
import com.hyp.blogmaster.pojo.vo.page.weixin.WeixinVoteUserWorkDiffVO;
import com.hyp.blogmaster.pojo.vo.page.weixin.WeixinVoteWorkSimpleVO;
...
public class WeixinVoteWorkServiceImpl implements WeixinVoteWorkService {
     ...
     */
    @Override
    public Integer getTotalUserWorkNum() {
        int i = 0;
        try {
            i = weixinVoteWorkMapper.selectCount(null);
        } catch (Exception e) {
            log.error("查询作品总数错误，错误理由：{}", e.toString());
            throw new MyDefinitionException("查询作品总数错误");
        }
        return i;
    }
